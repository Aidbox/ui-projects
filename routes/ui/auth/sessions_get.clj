      ...
      [:div.m-auto.flex.flex-column
       ...
       [:div.py-8.shadow.rounded
        ...
        [:div
         ...
         [:div.px-8
          [:div.text-xl.mb-4 "Sessions"]
          [:div.flex.flex-column.gap-2
           (for [session (box/sql ["select * from session where resource#>>'{user,id}' = ? order by cts desc" (:id user)])]
             [:div.flex.justify-between.gap-4
              [:div (:cts session)]
              (if (= (:id session) (:id (auth/session)))
                [:div.text-gray-500 "Current"]
                [:form.m-0 {:method "POST" :action (format "/ui/auth/sessions/%s/$drop?redirect=/ui/auth/sessions" (:id session))}
                 [:button.bg-red-500.py-1.px-4.text-white.rounded "Revoke"]])]
             )]

          [:br]

          #_[:form.m-auto {:method "POST" :action "/ui/auth/logout"}
