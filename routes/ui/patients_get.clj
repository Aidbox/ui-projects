[:html
 ...
 [:script {:type "module"} "import hotwiredTurbo from 'https://cdn.skypack.dev/@hotwired/turbo';"]
...
"}
    ...
    [:div.mx-auto.flex.flex-column

     [:div.shadow.rounded
      {:style "backdrop-filter: blur(8px); background: rgba(255, 255, 255, 0.6)"}
      [:div
       #_[:img.px-16.mb-4.m-auto {:src "/static/assets/img/aidbox-logo.svg" :style "max-width: 100%"}]

       [:div.flex.justify-between.w-full.mb-4
        #_(for [link [{:label "userinfo" :url "/ui/auth/userinfo"}
